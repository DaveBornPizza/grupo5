...
// Scheduling, especialmente: o que acontece se um processo pedir 0 bilhetes?
// E valores negativos? O processo sofre starvation ou o kernel quebra?
//
// Uso: testerobustez
...
main(void)
{
  ...
  // Caso 3: valor válido deve ser ACEITO (retorno 0).
  int rv = settickets(10);
  printf("settickets(10)  -> %d  %s\n", rv,
         rv == 0 ? "ACEITO (ok)" : "FALHA!");

  // Caso 4: mesmo após um pedido inválido, o processo continua executável
